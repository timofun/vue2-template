  ...
  <fn-dialog v-if="visible" :visible.sync="visible" :title="title" @save="onSave" :show-save="type !== 'detail'">
    <div>
      <el-form
        ...
        <el-row :gutter="119" justify="space-around">
          <el-col :span="12">
            <el-form-item label="设备名称" prop="facilityName">
              <el-input v-model="ruleForm.facilityName" placeholder="请输入"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row :gutter="119" justify="space-around">
          <el-col :span="12">
            <el-form-item label="图标上传" prop="facilityIcon">
              <fn-img-upload
                :url.sync="ruleForm.facilityIcon"
 ...
 */
import { addGymFacilitySetting, editGymFacilitySetting, getGymFacilitySettingDetail } from '@/api/gym-manage'
...
export default {
  ...
  async created () {
    ...
    if (this.type !== 'add') {
      const { data } = await getGymFacilitySettingDetail(this.id)
  ...
  methods: {
    onSave () {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          if (this.type === 'add') {
            addGymFacilitySetting(this.ruleForm).then((res) => {
            ...
            editGymFacilitySetting({ id: this.id, ...this.ruleForm }).then((res) => {
              if (res.data.code === REQ_OK) {
                this.$message.success(res.data.message)
                this.$emit('success')
                this.visible = false
