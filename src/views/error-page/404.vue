  ...
  <div class="wscn-http404-container">
    <div class="wscn-http404">
      ...
      <div class="bullshit">
        ...
        <router-link :to="getCurrRoute" class="bullshit__return-home">
          返回首页
        </router-link>
      </div>
    </div>
  ...
  computed: {
    ...
    getCurrRoute () {
      if (store.getters.routers.length) {
        const parentPath = store.getters.routers[0].path
        if (store.getters.routers[0].children && store.getters.routers[0].children.length) {
          return `${parentPath}/${store.getters.routers[0].children[0].path}`
        } else {
          return `${parentPath}`
        }
      }
      return '/gym/gym-setting'
    }
  }
}
</script>
